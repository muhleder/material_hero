...
class OpenContainer<T extends Object?> extends StatefulWidget {
  ...
  const OpenContainer({
    ...
    this.openAlignment = Alignment.topLeft,
    this.onClosed,
...
class OpenContainerState<T> extends State<OpenContainer<T?>> {
  ...
  Future<void> openContainer() async {
    ...
    final constrainedOpenSize = Size(
      min(widget.openSize.width, mediaQueryData.size.width - widget.openPadding.horizontal),
      min(widget.openSize.height, mediaQueryData.size.height - widget.openPadding.vertical),
    );
...
class _OpenContainerRoute<T> extends ModalRoute<T> {
  ...
  void _takeMeasurements({
    ...
    final double leftFactor = (openAlignment.x - 1) / 2;
    final double rightFactor = (openAlignment.x + 1) / 2;
    final double topFactor = (openAlignment.y - 1) / 2;
    final double bottomFactor = (openAlignment.y + 1) / 2;
    final alignOffset = Offset(
      openPadding.right * rightFactor + openPadding.left * leftFactor,
      openPadding.bottom * bottomFactor + openPadding.top * topFactor,
    );
    _rectTween.end = (Offset.zero - alignOffset) & Size(openSize.width, openSize.height);

    void takeMeasurementsInSourceRoute([Duration? _]) {
      if (!navigator.attached || hideableKey.currentContext == null) {
        return;
      }
      final Rect begin = _getRect(hideableKey, navigator);
      final startSize = begin.size;
      final Size navSize = _getSize(navigator);
      final double xFactor = (openAlignment.x + 1) / 2;
      final double yFactor = (openAlignment.y + 1) / 2;
      final Offset navOffset = Offset(xFactor * navSize.width, yFactor * navSize.height);
      final Offset startOffset = Offset(xFactor * startSize.width, yFactor * startSize.height);
      _rectTween.begin = begin.shift(-navOffset).shift(startOffset);
